...
| Precise location | Yes | No | App functionality | Read at check-in to verify presence; not stored as history |
| Coarse location | Yes | No | App functionality | Map centering |
...
- **Play data safety:** all collection marked "required for app
  ...
  transit, account + data deletable in-app (Profile → Delete account) and
  via the web app.
